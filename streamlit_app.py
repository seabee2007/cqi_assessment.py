...
if st.button("Calculate Final Score"):
    ...
    if 'item4_score' not in st.session_state:
        # Calculate Item 4 score using the input values.
        if int(total_md) < 1000:
            allowed = 10
        elif int(total_md) < 2000:
            allowed = 5
        else:
            allowed = 2.5
        diff = abs(int(actual_wip) - int(planned_wip))
        if diff == 0:
    ...
    if st.session_state.item4_score != 16 and not form_data.get("Comment for Item 4", "").strip():
    ...
    if errors:
        ...
        score15 = 2 if form_data["Item 15 – Report Submission"] == "Yes" else 0
