...
st.write("Fill out your project and checklist data below. For any item that does not achieve the perfect score, a comment is required.")
...
for item in handbook_info:
    ...
    if item in yes_no_items:
        ...
        # Require comment if response is not perfect (i.e. not "Yes").
        ...
        comment = st.text_area(
            f"Enter comment for {item} (required if score is below {perfect_scores.get(item, 'perfect')}):",
            key="Comment for " + item
        ) if score < perfect_scores.get(item, score) else ""
        form_data["Comment for " + item] = comment

# Calculate final score.
# This calculation assumes all scores are numeric.
